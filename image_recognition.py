...
class LossHistory(keras.callbacks.Callback):
    ...
    def loss_plot(self, loss_type):
        ...
        plt.title('epoch='+str(epochs)+',lr='+str(lr)+',batch_size='+str(batch_size)+'\nactivation='+activation+',nb_classes='+str(nb_classes)+',nb_per_class='+str(number_per_category))
        plt.grid(True)
...
def main():
    ...
    for i in range(0,nb_classes*test_per_category):
        if y_test[i] != classes[i]:
            ...
            print(category_test, '被错误分成', category_class);
    print('All:'+str(len(classes)))
    print('Wrong:'+str(wrong))
    print('Right:'+str(len(classes)-wrong))
